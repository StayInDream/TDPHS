...
namespace Spate
{
    ...
    public class NetManager : BaseManager
    {
        ...
        //    DataManager.ClearUpdateCacheData();
            ...
            {
                MLogger.LogNet(LogType.Error, "上一条消息尚未处理完毕", mFrontRequest.ToString());
                return false;
            }
        ...
        private static void DoPost(HttpRequest req)
        {
            ...
            if (req.IsFront)
            {
                MLogger.LogNet(LogType.Log, "发送:" + req.ToString());
                ...
                case NetState.Error:
                    {
                        ...
                        MLogger.LogNet(LogType.Error, "接收(Error):" + www.error);
                    }
                    break;
                case NetState.Timeout:
                    {
                        ...
                        MLogger.LogNet(LogType.Error, "接收(Timeout)");
                    }
                    break;
        ...
        private static object TranslateAsync(object arg)
        {
            ...
            // 网络日志
            if (req.IsFront)
                MLogger.LogNet(LogType.Log, "接收:" + orgText);
            // Json化
            ...
            if (req.respData != null)
            {
                ...
                if (data.ContainsKey("redirectUrl"))
                {
                    ...
                    NetHost.SetUrl(newHost);// 绑定最新的Host
                }
