...
namespace MiniJson
{
    ...
    public static class Json
    {
        ...
        public static bool ToObject(object input, object output, Action<string, object, object> valueUpdateHandle = null)
        {
            ...
            {
                fields.AddRange(output.GetType().GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly));
                fields.AddRange(output.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly));
            }
            ...
            foreach (MemberInfo field in fields)
            {
                ...
                {
                    Type fieldType = BaseData.GetMemberType(field);
                    if (ToField(fieldType, out fieldValue, dicValue))
                    {

                        // 取得旧值和新值进行判定，一旦发生改变就进行通知
                        //object oldFieldValue = field.GetValue(output);
                        object oldFieldValue = BaseData.GetMemberValue(field, output);
                        //field.SetValue(output, fieldValue);
                        BaseData.SetMemberValue(field, output, fieldValue);
                        if (valueUpdateHandle != null && !string.Equals(oldFieldValue + "", fieldValue + ""))
