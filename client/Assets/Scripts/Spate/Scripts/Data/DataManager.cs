...
namespace Spate
{
    public sealed class DataManager : BaseManager
    {
        ...
        public static void FillUpdateData( Type t, Dictionary<string, object> jsonData )
        {
            ...
            MemberInfo primaryKeyFiled = baseData.GetPrimaryKey();
            ...
            else
            {
                ...
                //{
                //    MemberInfo curFiled = oldData.GetCsvForeginKey();
                //    if(curFiled != null)
                //        BaseData.SetMemberValue( curFiled, baseData, BaseData.GetMemberValue( oldData.GetCsvForeginKey(), oldData ) );
                ...
                using(Dictionary<string, object>.Enumerator erator = jsonData.GetEnumerator())
                {
                    while(erator.MoveNext())
                    {
                        KeyValuePair<string, object> pair = erator.Current;
                        //
                        MemberInfo[] oldFileds = oldData.GetType().GetMember( pair.Key, BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly );
                        // FieldInfo oldFiled = oldData.GetType().GetField(pair.Key);
                        if(oldFileds.Length == 0)
                            continue;
                        MemberInfo oldFiled = oldFileds[0];
                        if(!pair.Value.Equals( BaseData.GetMemberValue( oldFiled, oldData ) ))
                        {
                            object addValue = null;
                            if(BaseData.GetMemberType( oldFiled ).IsValueType)
                            {
                                switch(Type.GetTypeCode( BaseData.GetMemberType( oldFiled ) ))
                                {
                                    case TypeCode.Int32:
                                        {
                                            int newValue = (int)pair.Value;
                                            int oldValue = (int)BaseData.GetMemberValue( oldFiled, oldData );
                                    ...
                                    case TypeCode.Single:
                                        addValue = (float)pair.Value - (float)BaseData.GetMemberValue( oldFiled, oldData );
                                        break;
                                    case TypeCode.Double:
                                        addValue = (double)pair.Value - (double)BaseData.GetMemberValue( oldFiled, oldData );
                                        break;
                                }
                            }
                            if(addValue != null)
                            {
                                isChange = true;
                                BaseData.SetMemberValue( oldFiled, baseData, addValue );
        ...
        private static void TryAutoUserData( BaseData svrData )
        {
            ...
            if(mAutoDataMap.ContainsKey( svrDataType ))
            {
                ...
                object primaryKey = BaseData.GetMemberValue( entity.svrPrimaryKey, svrData );
                ...
                else
                {
                    ...
                    object foreginKeyValue = BaseData.GetMemberValue( entity.svrCsvForeginKey, svrData );
        ...
        /// 移除非CSVData
        /// </summary>
        public static void RemoveAllExclusionCsvData()
        {
            Type csvType = typeof( BaseCsvData );
            Type appinfo = typeof( AppinfoSvrData );
            List<Type> list = new List<Type>();

            Dictionary<Type, Dictionary<object, BaseData>>.Enumerator erator = mDataBase.GetEnumerator();
            while(erator.MoveNext())
            {
                Type typeWillBeRemoved = erator.Current.Key;
                if(!typeWillBeRemoved.IsSubclassOf( csvType ) && !typeWillBeRemoved.Equals( appinfo ))
                    list.Add( typeWillBeRemoved );
            }
        ...
        private static Type TryFindSvrDataType( string name )
        {
            ...
            if(!mNameDataTypeMap.TryGetValue( name, out t ))
            {
                ...
                if(t != null && !mAutoDataMap.ContainsKey( t ))
                {
                    ...
                    MemberInfo primaryKey = baseData.GetPrimaryKey();
                    MemberInfo csvForeginKey = baseData.GetCsvForeginKey();
        ...
        private static Dictionary<object, BaseData> FillCsvData( Type t, string[] allLines )
        {
            ...
            for(int i = 3; i < allLines.Length; i++)
            {
                ...
                catch(Exception ex)
                {
                    ...
                    MLogger.LogAsset( LogType.Error, message );
                    break;
                }
            }
            return tableData;
        ...
        private static object GetMemberValue( BaseData baseData, KeyValuePair<string, object> pair )
        {
            object value = null;
        ...
        private class AutoEntity
        {
            ...
            public MemberInfo svrPrimaryKey;
            public MemberInfo svrCsvForeginKey;
        } 
    }
}
