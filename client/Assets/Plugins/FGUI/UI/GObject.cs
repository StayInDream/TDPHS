...
namespace FairyGUI
{
    public class GObject : EventDispatcher
    {
        ...
        public static explicit operator GObject( Controller v )
        {
        ...
        virtual public IFilter filter
        {
            get
            {
                return displayObject != null ? displayObject.filter : null;
            }
            set
            {
                if ( displayObject != null )
        ...
        virtual public BlendMode blendMode
        {
            get
            {
                return displayObject != null ? displayObject.blendMode : BlendMode.None;
            }
            set
            {
                if ( displayObject != null )
        ...
        /// 通用动画
        /// </summary>
        /// <param name="openOrclose">打开或者关闭</param>
        /// <param name="animType">动画类型</param>
        /// <param name="callback">动画显隐的回调函数</param>
        public void PlayAnim( AnimType animType , TweenCallback callback = null )
        {
            switch ( animType )
            {
                case AnimType.None:
                    callback();
                    break;
                case AnimType.ShowToBiggerCenter:
                    SetScale( 0.1f , 0.1f );
                    SetPivot( 0.5f , 0.5f );
                    TweenScale( new Vector2( 1 , 1 ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.ShowToBiggerFromLeftUp:
                    SetScale( 0.1f , 0.1f );
                    SetPivot( 0 , 0 );
                    TweenScale( new Vector2( 1 , 1 ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;

                case AnimType.ShowToBiggerFromLeftBottom:
                    SetScale( 0.1f , 0.1f );
                    SetPivot( 0 , 1 );
                    TweenScale( new Vector2( 1 , 1 ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.ShowToBiggerFromRightUp:
                    SetScale( 0.1f , 0.1f );
                    SetPivot( 1 , 0 );
                    TweenScale( new Vector2( 1 , 1 ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.ShowToBiggerFromRightBottom:
                    SetScale( 0.1f , 0.1f );
                    SetPivot( 1 , 1 );
                    TweenScale( new Vector2( 1 , 1 ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.ShowFromTop:
                    SetPosition( 0 , -GRoot.inst.height * 1.2f , 0 );
                    SetPivot( 0.5f , 0.5f );
                    TweenMove( new Vector2( 0 , 0 ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.ShowFromBottom:
                    SetPosition( 0 , GRoot.inst.height * 1.2f , 0 );
                    SetPivot( 0.5f , 0.5f );
                    TweenMove( new Vector2( 0 , 0 ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.ShowFromLeft:
                    SetPosition( -GRoot.inst.width * 1.2f , 0 , 0 );
                    SetPivot( 0.5f , 0.5f );
                    TweenMove( new Vector2( 0 , 0 ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.ShowFromRight:
                    SetPosition( GRoot.inst.width * 1.2f , 0 , 0 );
                    SetPivot( 0.5f , 0.5f );
                    TweenMove( new Vector2( 0 , 0 ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.ShowFade:
                    this.alpha = 0;
                    // SetPivot( 0.5f , 0.5f );
                    TweenFade( 1 , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;



                case AnimType.HideToSmallInCenter:
                    SetPivot( 0.5f , 0.5f );
                    TweenScale( new Vector2( 0.01f , 0.01f ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.HideToSmallInLeftUp:
                    SetPivot( 0 , 0 );
                    TweenScale( new Vector2( 0.01f , 0.01f ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.HideToSmallInLeftBottom:
                    SetPivot( 0 , 1 );
                    TweenScale( new Vector2( 0.01f , 0.01f ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.HideToSmallInRightUp:
                    SetPivot( 1 , 0 );
                    TweenScale( new Vector2( 0.01f , 0.01f ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.HideToSmallInRightBottom:
                    SetPivot( 1 , 1 );
                    TweenScale( new Vector2( 0.01f , 0.01f ) , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.HideToTop:
                    SetPivot( 0.5f , 0.5f );
                    TweenMoveY( -GRoot.inst.height * 1.2f , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.HideToBottom:
                    SetPivot( 0.5f , 0.5f );
                    TweenMoveY( GRoot.inst.height * 1.2f , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.HideToLeft:
                    SetPivot( 0.5f , 0.5f );
                    TweenMoveX( -GRoot.inst.width * 1.2f , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.HideToRight:
                    SetPivot( 0.5f , 0.5f );
                    TweenMoveX( GRoot.inst.width * 1.2f , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
                case AnimType.HideFade:
                    // SetPivot( 0.5f , 0.5f );
                    TweenFade( 0 , 0.3f ).SetEase( Ease.OutQuad ).OnComplete( callback );
                    break;
...
public enum AnimType
{
    ...
    /// 从小到大，从左上角出现
    /// </summary>
    ShowToBiggerFromLeftUp,
    /// <summary>
    /// 从小到大，出现在右上角
    /// </summary>
    ShowToBiggerFromRightUp,
    /// <summary>
    /// 从小到大，出现在右下角
    /// </summary>
    ShowToBiggerFromRightBottom,
    /// <summary>
    /// 从小到大，出现在左下角
    /// </summary>
    ShowToBiggerFromLeftBottom,
    /// <summary>
    /// 从顶部下拉出现
    /// </summary>
    ShowFromTop,
    ...
    /// 以垂直中线为转轴，从右向左旋转显现
    /// </summary>
    ShowRatateFromRightToLeft,
    /// <summary>
    /// 以垂直中线为转轴，从左向右旋转显现
    /// </summary>
    ShowRatateFromLeftToRight,
    /// <summary>
    /// 以水平中线为转轴，从上向下旋转显现
    /// </summary>
    ShowRatateFromTopToBottom,
    /// <summary>
    /// 以水平中线为转轴，从下向上旋转显现
    /// </summary>
    ShowRatateFromBottomToTop,
    ...
    ///  从大到小,消失在右下角
    /// </summary>
    HideToSmallInRightBottom,
    /// <summary>
    ///  从大到小,消失在左下角
    /// </summary>
    HideToSmallInLeftBottom,
    /// <summary>
    /// 上拉消失
    /// </summary>
    HideToTop,
    ...
    /// 以垂直中线为转轴，从右向左旋转消失
    /// </summary>
    HideRatateFromRightToLeft,
    /// <summary>
    /// 以垂直中线为转轴，从左向右旋转消失
    /// </summary>
    HideRatateFromLeftToRight,
    /// <summary>
    /// 以水平中线为转轴，从上向下旋转消失
    /// </summary>
    HideRatateFromTopToBottom,
    /// <summary>
    /// 以水平中线为转轴，从下向上旋转消失
    /// </summary>
    HideRatateFromBottomToTop,
}
